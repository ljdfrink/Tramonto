...
/*  if (Ndim==1 && Iwrite_files!=FILES_BASIC){*/
  ...
  if (!first && Proc==0 && Iwrite_screen != SCREEN_NONE && Iwrite_screen != SCREEN_ERRORS_ONLY) {
     if (Iwrite_screen==SCREEN_VERBOSE) printf("---------------------- FREE ENERGY ----------------------------\n");
  }
 ...
 /*        omega_surface_charge=integrateOverSurface(&integrand_surface_charge,iunk,x,Integration_profile);
         ...
         */
         ...
         if (Type_coul == DELTAC_RPM || Type_coul==DELTAC_GENERAL){     
            ...
            if (Proc==0 && Iwrite_screen == SCREEN_VERBOSE) print_to_screen(omega_MSA,"MSA CORRECTIONS");
            ...
            if (Proc==0 && Iwrite_screen == SCREEN_VERBOSE) print_to_screen(omega_MSA_surf_ex,"SURF.EX.: MSA CORRECTIONS");
       ...
       if (Type_poly == WTC){
          ...
          if (Proc==0 && Iwrite_screen == SCREEN_VERBOSE) print_to_screen(omega_WTC,"WTC BONDS");
          ...
          if (Proc==0 && Iwrite_screen == SCREEN_VERBOSE) print_to_screen(omega_WTC_surf_ex,"SURF.EX.: WTC BONDS");
       ...
       if (Type_poly == WJDC || Type_poly==WJDC2 || Type_poly==WJDC3){
        if(Grafted_Logical==FALSE){
          ...
          if (Lseg_densities){
             ...
             if (Proc==0 && Iwrite_screen == SCREEN_VERBOSE) print_to_screen(omega_WJDC,"WJDC BONDS");
             ...
             if (Proc==0 && Iwrite_screen == SCREEN_VERBOSE) print_to_screen(omega_WJDC_surf_ex,"SURF.EX.: WJDC-DTERM");
          }
          else{
